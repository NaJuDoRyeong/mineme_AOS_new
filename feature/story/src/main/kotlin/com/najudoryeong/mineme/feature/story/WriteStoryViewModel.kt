...
class WriteStoryViewModel @Inject constructor(
    ...
    val selectedCity: MutableStateFlow<String> =
        MutableStateFlow(koreanCities[koreanProvinces.first()]?.first() ?: "")
    ...
    init {
        viewModelScope.launch {
            selectedRegion.collectLatest { region ->
                val cities = koreanCities[region] ?: emptyList()
                allCities.value = cities
                updateCity(cities.firstOrNull() ?: "")
            }
        }
    }

    fun updateLocation(region: String, city: String) {
        viewModelScope.launch {
            updateRegion(region)
            yield()
            updateCity(city)
